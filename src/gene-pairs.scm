...
"
	(let ((all-pairs '()))
		...
		(define (do-get-all-pairs)
			...
			; hey, both left and right better be genes.
		   (filter!
				(lambda (pair)
					(and
		...
		; fetch-gene-pairs -- fetch all counts for gene pairs
...
"
	(let ((all-pairs '()))
		...
		(define any-right (AnyNode "right-gene"))
		(define pair-pred (PredicateNode "expresses"))
		...
		; get-all-pairs - return a list holding all of the observed
		; expression-pairs.  Caution: this can be tens of millions long!
		(define (do-get-all-pairs)
			; The list of pairs is mostly just the incoming set of the
			; predicate node. However, this does include some junk, sooo ...
			; hey, both left and right better be the right type.
		   (filter!
				(lambda (pair)
					(and
						(equal? 'GeneNode (cog-type (gadr pair)))
						(equal? 'MoleculeNode (cog-type (gddr pair)))))
		...
		(define (fetch-gene-pairs)
			...
			(format #t "Elapsed time to load gene-protein pairs: ~A secs\n"
				(- (current-time) start-time))
		)
		...
		; deleted; if any are hiding in the database, they will not be
		; touched.
		(define (delete-expression-pairs)
			(define start-time (current-time))
			(for-each (lambda (PAIR) (cog-delete-recursive (gdr PAIR)))
				(cog-incoming-set pair-pred))
			(cog-delete pair-pred)
			(cog-delete any-left)
			(cog-delete any-right)
			(format #t "Elapsed time to delete gene-protein pairs: ~A secs\n"
				(- (current-time) start-time))
		)
		...
		(lambda (message . args)
			(apply (case message
					((name) (lambda () "Gene-Protein pairs, predicate `expresses`"))
					((id)   (lambda () "Gene-expression"))
					((left-type) get-left-type)
					((right-type) get-right-type)
					((pair-type) get-pair-type)
					((pair-count) get-pair-count)
